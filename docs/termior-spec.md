...
| ID | 需求 | 优先级 |
|---|---|---|
| FR-SSH-01 | 标签栏新建菜单、系统偏好设置和左侧 SSH/SFTP 面板提供连接管理入口；左侧保存连接采用紧凑单行列表，默认仅列 SSH 入口；单击选中、左键双击连接 SSH，右键提供连接 SSH、以 SFTP 连接、重命名、编辑和确认删除；管理窗口支持小窗口滚动、可见滚动条和自然宽度光标；支持配置新建、选择、编辑、保存和确认删除；字段包含名称、主机/SSH config 别名、用户名、端口、私钥和多级跳板机 | P1 |
