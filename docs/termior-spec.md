...
- NG3：不把本地 Agent 或 Git 隐式切换为远程执行。文件浏览器在活动 Tab 为用户显式建立的 SSH/SFTP 会话时，按 FR-SSH-07 切换为该主机的 SFTP 上下文；这不构成远程 Agent 或完整远程工作区。远程 Agent/完整远程工作区与云端托管仍不在本轮范围。
...
| FR-WS-08 | 每个 tab 持有独立的项目文件夹（`project_dir`）：打开文件夹（标题栏/状态栏按钮、`Cmd/Ctrl+O`）只重定向**活动 tab**，其他 tab 不受影响；运行中的本地终端注入 `cd` 同步（会话不中断）；本地 Tab 的文件浏览器、Git 面板与状态栏工作区名跟随活动 tab 的 `project_dir`；`project_dir` 不随 shell `cd`（OSC 7）漂移；活动 SSH/SFTP Tab 的文件浏览器改按 FR-SSH-07 显示远程 SFTP 上下文，但 Git、Agent 与本地 `project_dir` 不切换；全局 `root` 降为兑底（新建 tab 无继承来源、private terminal 使用） | P0 |
...
| FR-TERM-11 | 终端右键菜单提供复制、粘贴、全选（含回滚历史）、查找及分屏操作；无选区时禁用复制，操作绑定右键所在终端；鼠标报告模式下用 Shift+右键打开本地菜单 | P0 |
...
| ID | 需求 | 优先级 |
|---|---|---|
| FR-SSH-01 | 标签栏新建菜单、系统偏好设置和左侧 SSH/SFTP 面板提供连接管理入口；左侧保存连接采用紧凑单行列表，默认仅列 SSH 入口；单击选中、左键双击连接 SSH，右键提供连接 SSH、以 SFTP 连接、重命名、编辑、移动到分组和确认删除；连接可归属一个持久化分组或未分组，未分组在前、分组按存储顺序展示，分组表头可折叠，分组可新建（含空分组）、重命名（与现有分组重名即合并）和删除（成员归入未分组），分组独立于连接持久化；管理窗口提供分组字段，可就地新建分组，支持小窗口滚动、可见滚动条和自然宽度光标；支持配置新建、选择、编辑、保存和确认删除；字段包含名称、主机/SSH config 别名、用户名、端口、私钥和多级跳板机 | P1 |
...
| FR-SSH-05 | SFTP 会话默认打开本地/远程双栏图形文件浏览器，以名称/类型/大小列展示，目录双击进入；支持跨栏及系统文件管理器拖放上传/下载（含目录递归），拖到目录行以该目录为目标；远程右键提供打开、新建、重命名、移动、删除、上传/下载和刷新。保留图形文件选择式传输、显式续传及覆盖前确认；传输在独立标签显示进度与错误，关闭/断开可取消，成功由进程退出码判定 | P1 |
...
| FR-SSH-07 | 活动 Tab 是已连接的 SSH 会话时，左侧 File Explorer SHALL 切换到同一配置的远程 SFTP 目录；独立 SFTP Tab 在主区域提供双栏文件浏览，左侧保留会话入口而不重复渲染远程列表；优先跟随该远程终端经 OSC 7 报告的 cwd，无报告时从远程登录目录开始。切换回本地 Tab SHALL 恢复该 Tab 的本地树。远程视图提供目录导航、新建、重命名、移动、删除、上传与下载；覆盖前确认，目录删除有界且递归。远程路径只作为该 Tab 的远程元数据，不写入本地 `project_dir`，不驱动本地 Git/Agent/localhost 上下文 | P1 |
| FR-SSH-08 | Remote Explorer SHALL 在每个 Tab 内复用后台 SFTP 连接；Windows 不创建独立控制台窗口，正常密码/MFA/主机指纹确认仍明确显示。目录缓存有界且短期有效，刷新绕过缓存，写操作使缓存失效；重复/快速导航合并为当前请求及最新待处理路径。请求可取消、有总时限，连接/读取/操作进度及错误显示在侧栏；关闭/断开 Tab 终止后台连接。失败或中断的写操作不得自动重放，并提示可能已产生部分远程副作用 | P1 |
| FR-SSH-09 | 同一凭据目标的桌面 SSH/SFTP 终端、Explorer 和传输 SHALL 共用认证协调器与单一图形认证入口，普通密码/指定私钥口令并发请求只询问一次，不再同时在终端索要密码。默认只在存活会话内存复用；认证窗口及连接管理可显式启用记住密码，首次输入保存至系统凭据库，后续显式连接自动读取。失效密码提示重新输入；取消、会话结束和超时关闭等待。OTP/未知 keyboard-interactive/代理跳板提示不复用或保存；首次主机信任仍明确确认，变更密钥拒绝连接 | P1 |
...
**验收标准**：真实 OpenSSH 客户端通过应用 PTY 连接隔离测试服务；密钥与密码登录成功；主机密钥变化失败；文件上传下载字节一致；中文、空格、方括号文件名准确；部分文件续传及目录传输通过；活动 SSH Tab 的 Explorer 显示远程 cwd，切回本地 Tab 恢复本地树；远程新建、重命名、移动和删除生效；远端 OSC 不影响本地项目、Git、Agent 或 localhost 上下文；失败保留退出码与输出。
...
| `Termior-ssh.json` | SSH 连接配置 schema v2（连接分组 `groups` + 连接引用 `group`；v1 读取时自动迁移）；不含密码、私钥内容、口令或 OTP |
