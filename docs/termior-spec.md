...
| FR-SSH-07 | 活动 Tab 是已连接的 SSH/SFTP 会话时，左侧 File Explorer SHALL 切换到同一配置的远程 SFTP 目录；优先跟随该远程终端经 OSC 7 报告的 cwd，无报告时从远程登录目录开始。切换回本地 Tab SHALL 恢复该 Tab 的本地树。远程视图提供目录导航、新建、重命名、移动、删除、上传与下载；覆盖前确认，目录删除有界且递归。远程路径只作为该 Tab 的远程元数据，不写入本地 `project_dir`，不驱动本地 Git/Agent/localhost 上下文 | P1 |
