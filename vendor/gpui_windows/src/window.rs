...
impl Drop for WindowsWindow {
    fn drop(&mut self) {
        // Drop all platform callbacks before scheduling DestroyWindow.
        // Otherwise, after GPUI removes the window from App.windows but while the
        // HWND is still alive (DestroyWindow is deferred), WM_ACTIVATE / WM_PAINT
        // can still invoke these callbacks and `handle.update().log_err()` logs a
        // spurious `window not found`. Clearing here also prevents a second
        // DestroyWindow (from DefWindowProc close) from racing the same callbacks.
        self.0.state.callbacks.request_frame.take();
        self.0.state.callbacks.input.take();
        self.0.state.callbacks.active_status_change.take();
        self.0.state.callbacks.hovered_status_change.take();
        self.0.state.callbacks.resize.take();
        self.0.state.callbacks.moved.take();
        self.0.state.callbacks.should_close.take();
        self.0.state.callbacks.close.take();
        self.0.state.callbacks.hit_test_window_control.take();
        self.0.state.callbacks.appearance_changed.take();
        ...
        self.0
            ...
            .spawn(async move {
                ...
                unsafe {
                    // Skip if Win32 already destroyed the HWND (DefWindowProc path).
