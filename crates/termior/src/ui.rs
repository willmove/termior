//! 共享 UI 层:全局主题访问 + 统一控件样式。
//!
//! 所有视图(含独立的设置窗口)通过 [`palette`] 读取当前主题色板,
//! 主题切换时 [`set_palette`] 更新全局并刷新全部窗口,保证浅色/深色
//! 主题下每个界面都能正确着色 —— 禁止在视图里硬编码 rgba 颜色。

use gpui::{div, prelude::*, App, Div, Global, Rgba, SharedString, Stateful};
use termior_theme::{Appearance, Color, ResolvedPalette};
...
pub fn palette(cx: &App) -> ResolvedPalette {
    cx.try_global::<ActiveTheme>()
        ...
        .unwrap_or_else(|| termior_theme::default_theme().resolve(Appearance::Dark, true))
}
...
/// 实色按钮悬浮时的提亮/压暗色。
fn hover_shade(value: Color) -> Color {
    if luminance(value) > 0.6 {
